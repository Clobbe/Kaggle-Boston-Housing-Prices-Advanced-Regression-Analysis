...
Setting the segmentet data frames
...
segment.building.correlations <- df.train.segment.building %>% 
                                  select_if(is.numeric) %>%
                                  map(~cor(x = . , y = df.train.segment.building$SalePrice, method = 'pearson')) %>%
                                  as_data_frame() %>% 
                                  t() %>% 
                                  as.vector()

segment.location.correlations <- df.train.segment.location %>% 
                                  select_if(is.numeric) %>%
                                  map(~cor(x = . , y = df.train.segment.location$SalePrice, method = 'pearson')) %>%
                                  as_data_frame() %>% 
                                  t() %>% 
                                  as.vector()

segment.space.correlations <- df.train.segment.space %>% 
                                  select_if(is.numeric) %>%
                                  map(~cor(x = . , y = df.train.segment.space$SalePrice, method = 'pearson')) %>%
                                  as_data_frame() %>% 
                                  t() %>% 
                                  as.vector()

df.segments <- bind_rows(df.segments, )

df.segments.tmp <- data.frame(variable = df.train.segment.building %>% 
                                      select_if(is.numeric) %>% 
                                      colnames(),
                  cor_values = segment.building.correlations,
                  segment = 'building')
