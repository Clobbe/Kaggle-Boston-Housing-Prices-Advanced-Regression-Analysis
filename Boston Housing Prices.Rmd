...
#calculating correlations and tiding the data frame
...
We're interested in filter out the noise that just messes up our plot so that we can reach what we're looking for - which variables correlate with increase on the target `SalePrice`. 
...
But first let's prep the data a bit to make it possible to plot what we want.
