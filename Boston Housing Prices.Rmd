...
Anyhow these 10 variables are correlated we an increase in price. Let's do some health check on each correlation to see that all of them is normally distributed or not. We'll do this with some plots in `ggplot2` and `qplot`.



1. plotta resp variabels  som bell-kurva + propability plot
2. normalisera de som inte är norm mot SalesPrice
...
df.train.filtered.model <- df.train.filtered %>% 
  lm(data, SalePrice ~ interestingVariables, .) %>% 
  mutate(model = tidy(.))
